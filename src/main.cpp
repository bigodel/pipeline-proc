...
int sc_main(int argc, char *argv[]) {
    ...
    sc_signal<bool> leq_result;
    leq leq("leq");
    leq.a(rf_data1);
    leq.b(rf_data2);
    leq.result(leq_result);
    ...
    while (inst_file >> opcode_name) {
        WORD instruction = opcode_from_string[opcode_name];
        ...
        } else if (itype || branch) { // i-type or branch
            int rs;
            int rt;
            int imm;
    ...
    for (int i = 0; i < 9; i++) {
        sc_trace(fp, register_file.registers[i],
                 "register_file|0-7-registers-" + std::to_string(i));
    }
    ...
    sc_trace(fp, id_ex_reg.mem_to_reg_out,
             "id_ex_reg|ctrl|1-26-mem_to_reg_out");
    ...
    for (int i = 0; i < 9; i++) {
        sc_trace(fp, data_memory.memory[i],
                 "data_memory|1-3-memory-" + std::to_string(i));
    }
    ...
    sc_trace(fp, ex_mem_reg.mem_to_reg_out,
             "ex_mem_reg|ctrl|1-9-mem_to_reg_out");
    sc_trace(fp, ex_mem_reg.reg_write_in, "ex_mem_reg|ctrl|1-10-reg_write_in");
    sc_trace(fp, ex_mem_reg.reg_write_out,
             "ex_mem_reg|ctrl|1-11-reg_write_out");
    ...
    sc_trace(fp, mem_wb_reg.mem_to_reg_out,
             "mem_wb_reg|ctrl|1-1-mem_to_reg_out");
