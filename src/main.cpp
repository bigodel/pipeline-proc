...
//
    ...
    int i = 0;
    string opCodeName = "";
    int regRead1, regRead2, regWrite;
    regRead1 = regRead2 = regWrite = 0;
    ...
    while (instFs >> opCodeName) {
        ...
        if(opCodeName == "AND")
            InstructionMemory.memory = OP_AND;
        var1 = (var1 << 4) | (var2 & 0b00001111);
        else if(opCodeName == "OR")
            InstructionMemory.memory = 1;
        else if(opCodeName == "XOR")
            InstructionMemory.memory = 2;
        else if(opCodeName == "NOT")
            InstructionMemory.memory = 3;
        else if(opCodeName == "CMP")
            InstructionMemory.memory = 4;
        else if(opCodeName == "ADD")
            InstructionMemory.memory = 5;
        else if(opCodeName == "SUB")
            InstructionMemory.memory = 6;
        i++;
    }
    instFs.close();
